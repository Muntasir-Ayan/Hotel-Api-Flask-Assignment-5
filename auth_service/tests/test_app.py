...
def test_profile_valid_token(mock_requests_get, mock_verify_token, client):
    ...
    response = client.get(
        "/auth/profile", headers={"Authorization": f"Bearer {VALID_USER_TOKEN}"}
    )
...
def test_profile_invalid_token(mock_verify_token, client):
    ...
    response = client.get(
        "/auth/profile", headers={"Authorization": f"Bearer {INVALID_TOKEN}"}
    )
...
def test_destinations_valid_user(mock_requests_get, mock_verify_token, client):
    ...
    response = client.get(
        "/auth/destinations", headers={"Authorization": f"Bearer {VALID_USER_TOKEN}"}
    )
    assert response.status_code == 200
    assert response.json == [{"id": 1, "name": "Paris"}, {"id": 2, "name": "New York"}]
...
def test_destinations_valid_admin(mock_requests_get, mock_verify_token, client):
    ...
    response = client.get(
        "/auth/destinations", headers={"Authorization": f"Bearer {VALID_ADMIN_TOKEN}"}
    )
    assert response.status_code == 200
    assert response.json == [{"id": 1, "name": "Tokyo"}, {"id": 2, "name": "London"}]
