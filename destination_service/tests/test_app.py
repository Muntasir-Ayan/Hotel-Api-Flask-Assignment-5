...
def test_add_duplicate_destination(client, admin_token):
    ...
    response = client.post('/destinations/', json=duplicate_destination, headers=headers)
...
def test_delete_destination(client, admin_token):
    ...
    assert response.status_code == 201, f"Expected 201, got {response.status_code}. Error: {response.json}"
    
    created_destination = response.get_json()
    destination_id = created_destination.get('id')  # Capture the ID of the newly added destination
...
def test_update_destination_invalid_data(client, admin_token):
    ...
    response = client.put('/destinations/New York', json=update_data, headers=headers)
    assert response.status_code == 400
    assert response.get_json()['message'] == 'At least one of description or location must be provided to update.'
